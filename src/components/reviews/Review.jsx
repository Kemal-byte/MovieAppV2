...
const Review = ({ movieId }) => {
  ...
  const baseUrlAvatar = `https://image.tmdb.org/t/p/original`;
  const API = `https://api.themoviedb.org/3/movie/${id}/reviews?api_key=81e74a5a3eda706f29bc7cfbd9013f25&language=en-US&page=1`;
      ...
      <Stack spacing={2}>
        {reviews &&
          reviews.slice(0, 5).map((item, index) => {
            return (
              <Item sx={{ padding: "1rem", color: "#fff" }} key={index}>
                <Box
                  sx={{
                    display: "flex",
                ...
                >
                  <Box sx={{ display: "flex", alignContent: "center" }}>
                    <Avatar
                      alt="Remy Sharp"
                      src={
                        `${baseUrlAvatar}${item?.author_details?.avatar_path}` ||
                    ...
                    <h5 className="my-auto px-2">
                      {item?.author_details?.name || "Movie Critic"}
                    </h5>
                  </Box>

                  <h6>Rated {item?.author_details?.rating}/10</h6>
                </Box>
                <Box sx={{ fontSize: "16px", paddingTop: "10px" }}>
                  {item?.content}
                </Box>
              </Item>
            );
